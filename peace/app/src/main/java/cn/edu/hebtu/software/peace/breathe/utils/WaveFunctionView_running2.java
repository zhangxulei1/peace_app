...
public class WaveFunctionView_running2 extends View {
...
//    private float waveDeep = 8f;//波峰与波谷s
    private float waveDeepmin = 20f;//最小的波峰与波谷
    private float waveDeepMax = 60f;//最大的波峰与波谷
private float waveDeep = 20f;//波峰与波谷
    private float arcRa = 0;//圆半径
    private Boolean iscircle = true;//是否是圆形图案
    private Boolean antiAlias = true;//是否开启抗锯齿
    public String MAINCOLOR_DEF = "#FFFAFA", NEXTCOLOR_DEF = "#FFF68F";//默认颜色
    ...
    public WaveFunctionView_running2(Context context) {
        super(context);
        init();
    }

    public WaveFunctionView_running2(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public WaveFunctionView_running2(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void ChangeWaveLevel(int percent) {
        ...
        animatorh.setDuration(4000);//设置动画时间
